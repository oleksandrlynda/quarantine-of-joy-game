...
- Mutation offers cannot be declined. All run ranks reset on death or restart.

## Classified Armory

Classified weapons use three distinct states: hidden, revealed, and owned. Before the reveal milestone, the Archive shows only a classified silhouette and the required milestone. Reaching that milestone reveals the weapon and grants one temporary trial for the current run. Buying its license makes it available in future runs.

| Weapon | Reveal and trial | Archive cost | Permanent result |
|---|---|---:|---|
...
- Once unlocked, it can appear in the Wave 1 offer of every future run.
...
- Final Cut takes priority over Opening Act if a regular wave contains only one enemy, preventing stacked overlays and death animations.
- Kill credit, drops, score, and wave completion resolve before the visual finishes. Bosses and tutorials are excluded.

## Permanent Weapon Mastery

Mastery is deliberately modest. Standard magazine upgrades do not change reserve ammo or pickup yield, so sustained ammo efficiency remains intact. Minigun Overdrive is the premium exception: its high costs also buy a larger starting reserve.

| Weapon | Grade 0 | Grade I | Grade II | Grade III |
|---|---:|---:|---:|---:|
| SMG magazine | 36 | 40 | 44 | 48 |
| SMG damage | 100% | 105% | 110% | 115% |
| Rifle magazine | 16 | 18 | 20 | 22 |
| Rifle damage | 100% | 105% | 110% | 115% |
...
## Permanent Combat Verb: Punchline Rush

- Archive cost: 10 Memefragments. It does not enter numerical mutation offers and has no run ranks.
- Press Q at full stamina to commit to a collision-aware 10-meter forward rush over 0.6 seconds.
- The player ignores damage only while the rush is active. Enemies touched are pushed 2.6 units and stunned for 1.5 seconds; each enemy can be affected once per rush.
- Bosses receive at most a light 0.8-unit push and cannot be stunned.
- Activation consumes the entire stamina bar. Regeneration waits 8 seconds after the rush, then uses the normal 18-per-second rate, creating roughly 13.6 seconds from activation to a full bar.
- Mobile builds expose a Q button only after the Archive unlock; the button disables until stamina is full. The ALT button appears only for weapons with an available alternate action.
...
## Deferred Counterbroadcast

Boss-specific failure assistance is intentionally outside v1. The locked follow-up rule is one optional temporary assist after three defeats against the same boss and a second different assist after five. Both clear on victory.
